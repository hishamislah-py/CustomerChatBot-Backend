...
| 21 | Credits (for deposit)  Credit transaction for deposit/ deposit memo.  For use on the STAR Network via Mastercard Network Exchange (MNE) only. | Yes |
| 22 | Credits - Card Load | Yes |
| 23 | Debits - Card Unload | Yes |
| 24 | Envelope-less Cash deposit at an Automated Teller Machine (ATM).  for use on the STAR Network via Mastercard Network Exchange (MNE) only. | Yes |
| 25 | Envelope-less Cash deposit at an Automated Teller Machine (ATM).  for use on the STAR Network via Mastercard Network Exchange (MNE) only. | Yes |
...
| 50 | Bill payment to another third party from a Deferred debit, Charge or Prepaid account.  For use on the STAR Network via Mastercard Network Exchange (MNE) only. | Yes |
| 54 | Bill payment to another third party from a Deferred debit, Charge or Prepaid account.  For use on the STAR Network via Mastercard Network Exchange (MNE) only. | Yes |
| 55 | Facilitated payment from Third Party into customer's bank account.  For use on the STAR Network via Mastercard Network Exchange (MNE) only. | No |
| 56 | Payment Credit for a declined credit card transaction.  For use on the STAR Network via Mastercard Network Exchange (MNE) only. | No |
